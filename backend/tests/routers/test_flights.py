...
def test_track_flight_status_success(mock_get_booking_details, client: TestClient):
    ...
    mock_get_booking_details.return_value = {
        ...
        "flights": [{"flight": {"origin": {"name": "JFK"}, "destination": {"name": "LAX"}, "departure_time": "2025-10-10T10:00:00", "arrival_time": "2025-10-10T13:00:00", "flight_number": "SBJ123"}}]
    }
    response = client.get(f"/flights/track/{booking_id}")
    assert response.status_code == 200
    assert "text/event-stream" in response.headers["content-type"]
    assert "event: flight_status" in response.text
    app.dependency_overrides.clear()

@patch("app.routers.flights.get_booking_details_by_id")
def test_track_flight_status_unauthorized_access(mock_get_booking_details, client: TestClient):
    ...
    assert response.json()["detail"] == "Booking not found or you do not have permission to view it."
    mock_get_booking_details.assert_called_once_with(booking_id, user_id=MOCK_USER["id"])
    app.dependency_overrides.clear()

@patch("app.routers.flights.get_booking_details_by_id")
def test_track_flight_status_booking_not_found(mock_get_booking_details, client: TestClient):
...
@patch("app.routers.flights.get_booking_details_by_id")
def test_track_flight_status_no_flight_details(mock_get_booking_details, client: TestClient):
    """Test tracking a booking that has no associated flight details."""
    app.dependency_overrides[get_current_user] = mock_get_current_user
    booking_id = "booking-with-no-flight"
    mock_get_booking_details.return_value = {"id": booking_id, "user_id": MOCK_USER["id"], "flights": []}
    response = client.get(f"/flights/track/{booking_id}")
    assert response.status_code == 404
